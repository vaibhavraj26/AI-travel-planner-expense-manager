...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    {{-- Free --}}
    <div class="{{ $animated ? 'reveal feature-card' : '' }} bg-gold-base border border-[#EDE1D8] rounded-3xl p-10 relative">
        <div class="absolute top-10 right-10 border border-gold/30 text-gold text-xs font-bold px-3 py-1 rounded-full">FREE</div>
        <div class="text-slate-400 text-sm font-semibold tracking-widest uppercase mb-3">Explorer</div>
        <div class="font-['Playfair_Display',serif] text-5xl font-black text-page-text mb-1">Free</div>
        <div class="text-slate-500 text-sm mb-8">Perfect to get started</div>
        <ul class="space-y-3 mb-10">
            @foreach(['3 Active Trips','Basic Itinerary Builder','Classic PDF Export', 'Community Support'] as $feature)
            ...
            @endforeach
            @foreach(['Budget & Expense Tracker', 'Explorer Rewards'] as $feature)
        ...
        @elseif($plusCurrent)
            <div class="btn-outline no-lift w-full block text-center py-3.5 rounded-full text-gold font-semibold text-sm">
                {{ $freeLabel }} 
            </div>
        @else
            <a href="{{ $freeUrl }}" class="btn-outline w-full block text-center py-3.5 rounded-full text-gold font-semibold text-sm">{{ $freeLabel }}</a>
        @endif
    </div>

    {{-- Pro --}}
    <div class="{{ $animated ? 'reveal feature-card' : '' }} relative bg-gradient-to-br from-[#FFF2E6] to-gold-base border border-[#FFB800]/30 rounded-3xl p-10 overflow-hidden">
        <div class="absolute top-10 right-10 bg-gold text-[#070B14] text-xs font-bold px-3 py-1 rounded-full">POPULAR</div>
        <div class="text-gold text-sm font-semibold tracking-widest uppercase mb-3">Explorer Plus</div>
        <div class="flex items-end gap-1 mb-1">
            <span class="font-['Playfair_Display',serif] text-5xl font-black text-page-text">$9</span>
            <span class="text-slate-400 text-sm mb-2">/month</span>
        </div>
        <div class="text-slate-500 text-sm mb-8">For serious travellers</div>
        <ul class="space-y-3 mb-10">
            @foreach(['Unlimited Trips','AI-Powered Builder','PDF Export + Offline Vault','Community Support','Budget & Expense Tracker','Explorer Rewards'] as $feature)
            <li class="flex items-center gap-3 text-slate-700 text-sm">
                <span class="w-5 h-5 rounded-full bg-gold/20 flex items-center justify-center flex-shrink-0">
                    <svg class="w-3 h-3 text-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                </span>
                {{ $feature }}
            </li>
            @endforeach
