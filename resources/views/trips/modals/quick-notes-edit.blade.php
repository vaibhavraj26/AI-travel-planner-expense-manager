        ...
        <div class="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl" @click.away="showQuickNotesEdit = false">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xl font-black text-page-text">Edit Quick Notes</h3>
                ...
                <form method="POST" action="{{ route('trips.quicknotes.update', $trip) }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="quick_notes" rows="4" class="w-full px-4 py-3 bg-slate-50 border-2 border-transparent rounded-xl focus:bg-white focus:border-party-1 outline-none transition-all">{{ old('quick_notes', $trip->quick_notes) }}</textarea>
                    </div>

                    <div class="mt-4 flex gap-3 justify-end">
                        <button type="button" @click="showQuickNotesEdit = false" class="px-4 py-2 rounded-lg border">Cancel</button>
                        <button type="submit" class="px-4 py-2 bg-party-1 text-white rounded-lg font-bold">Save</button>
                    </div>
                </form>
            </div>
        </div>
