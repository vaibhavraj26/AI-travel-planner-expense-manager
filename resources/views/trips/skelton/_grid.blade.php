...
@if(!isset($hasAnyTrips) || !$hasAnyTrips)
    <div class="bg-white rounded-3xl p-12 border border-dashed border-slate-300 text-center flex flex-col items-center justify-center space-y-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-page-text">No trips planned yet</h2>
        ...
        <a href="{{ route('trips.create') }}" class="btn-primary py-3 px-8 rounded-xl text-page-text font-bold shadow-lg shadow-party-1/20 hover:scale-105 transition-transform">
            Get Started
        </a>
    </div>
@elseif($trips->isEmpty())
    <div class="bg-white rounded-3xl p-12 border border-dashed border-slate-300 text-center flex flex-col items-center justify-center space-y-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-page-text">No trips found</h2>
            <p class="text-slate-500 mt-2 max-w-sm mx-auto">We couldn't find any trips matching your search or filters.
            Try different keywords or <a href="{{ route('trips.index') }}" class="text-party-1 font-bold">clear filters</a> to see all trips.</p>
        </div>
    </div>
@else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($trips as $trip)
            <a href="{{ route('trips.show', $trip) }}" class="group bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-party-1 focus:ring-offset-2">
                {{-- Card Image --}}
                ...
                <div class="p-6 flex-1 flex flex-col justify-between">
                    ...
                    <div class="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                        ...
                        <div class="text-sm font-bold text-party-1 group-hover:underline flex items-center gap-1">
                            Details
